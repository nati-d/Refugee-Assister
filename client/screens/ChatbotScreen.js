...
export default function ChatbotScreen() {
  ...
  const handleSend = async () => {
    try {
      console.log("Send button clicked"); // Log that the button was clicked

      if (!message) {
        console.log("Message is empty"); // Log if the message is empty
        return;
      }

      const response = await axios.post("http://196.168.1.8:3000/chat", { message: message });

      console.log("Request sent successfully"); // Log that the request was sent successfully

      if (response && response.data && response.data.response) {
        const ai = response.data.response;
        console.log("AI Response:", ai); // Log the AI response
      } else {
        console.log("Response data is missing or invalid"); // Log if the response data is missing or invalid
      }

      setMessage(""); // Clear the message input
    } catch (err) {
      console.log("Error:", err.message); // Log any errors that occur during the request
    }
  };
